...
import { ORG_GITHUB_URL, type ReduceMotion } from './ProgressShared';

export function ProgressContribute({ reduce }: { reduce: ReduceMotion }) {
  ...
  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(ORG_GITHUB_URL);
    ...
    >
      <div className="relative overflow-hidden rounded-3xl bg-sb-navy p-6 text-sb-cream shadow-[0_18px_40px_rgba(8,31,52,0.18)] ring-1 ring-sb-navy min-[880px]:p-10">
        ...
        <div className="relative flex flex-col gap-5">
          ...
          >
            Help us automate the next number.
          </motion.h2>
          <motion.p
            variants={revealUp}
            className="max-w-[58ch] text-[1.05rem] leading-[1.6] text-sb-cream/90"
          >
            Most numbers on this page are still hand-updated each week. The plan is to wire
            each metric directly to its source &mdash; bank feeds, GitHub APIs, signup flows
            &mdash; so the data updates itself and corrections show up in git history. If you
            can help us automate the next pipeline, we&rsquo;d love to talk.
          </motion.p>
          <motion.div variants={revealUp} className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 rounded-full bg-sb-navy-hot px-4 py-2 ring-1 ring-sb-cream/10">
              <code className="font-mono text-sm text-sb-cream">{ORG_GITHUB_URL}</code>
            </div>
            <button
              type="button"
              onClick={handleCopy}
